...
class PlaceBetModal(discord.ui.Modal, title="💰 Fazer Aposta"):
    ...
    def __init__(self, event_id: str, choice: int, event_title: str, option_name: str, callback: Optional[Callable] = None):
...
class UserRegistrationModal(discord.ui.Modal, title="📝 Registro de Usuário"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.callback:
            ...
            await interaction.response.send_message(
                f"✅ Bem-vindo, {self.username.value}!",
                ephemeral=True
            )
...
class EditBetModal(discord.ui.Modal, title="✏️ Editar Aposta"):
    ...
    def __init__(self, event_id: str, current_title: str, current_description: str, callback: Optional[Callable] = None):
        super().__init__()
        self.event_id = event_id
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.callback:
            await self.callback(
                ...
                description=self.new_description.value if self.new_description.value else None
            )
        else:
            await interaction.response.send_message(
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception):
        ...
        await interaction.response.send_message(
            "❌ Ocorreu um erro ao editar a aposta. Tente novamente.",
            ephemeral=True
        )
        ...
        try:
            ...
            if self.callback:
                await self.callback(
                    ...
                    amount=amount_value,
                    description=self.description.value if self.description.value else "Transferência de moedas"
                )
        except ValueError:
            await interaction.response.send_message(
                "❌ Por favor, insira um número válido!",
...
class AIPromptModal(discord.ui.Modal, title="🤖 Consultar IA"):
    ...
    def __init__(self, provider: Optional[str] = None, callback: Optional[Callable] = None):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.callback:
            await self.callback(
                ...
                system_prompt=self.system_prompt.value if self.system_prompt.value else None
            )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if x < -10 or x > 10 or y < -10 or y > 10:
                await interaction.response.send_message(
                    "❌ As coordenadas devem estar entre -10 e 10!",
                    ephemeral=True
                )
                return
