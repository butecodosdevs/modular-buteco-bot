...
def client_commands(bot):
    ...
    @bot.tree.command(
        name="registrar", description="Registrar-se no sistema usando interface modal"
    )
    async def registrar(interaction: discord.Interaction):
        """Register using a modal interface"""

        async def handle_registration(
            interaction: discord.Interaction, username: str, bio: str = None
        ):
            ...
            async with aiohttp.ClientSession() as session:
                user_data = {"discordId": discord_id, "username": username}
                ...
                if status == 200:
                    ...
                    error_msg = (
                        ...
                        else response.get("detail", "Erro desconhecido")
                    )
                    embed = discord.Embed(
