...
def ai_commands(bot):
    ...
    async def mestre(interaction: discord.Interaction, provider: Optional[str] = None):
        ...
        async def handle_ai_prompt(interaction: discord.Interaction, prompt: str, provider: Optional[str], system_prompt: Optional[str]):
            ...
            if status == 200 and response and isinstance(response, dict) and response.get("text"):
                ...
                if len(response_text) > max_length:
                    ...
                    embed.add_field(name="💬 Seu Prompt", value=prompt[:1024], inline=False)
                    if system_prompt:
                        embed.add_field(name="🎯 Orientação", value=system_prompt[:1024], inline=False)
                    embed.set_footer(text=f"Custo: {amount} moedas | Provider: {provider or 'padrão'}")
                    await interaction.followup.send(embed=embed)
